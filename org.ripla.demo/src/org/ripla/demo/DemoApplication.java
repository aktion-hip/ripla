...
public class DemoApplication extends RiplaApplication {
	...
	protected Window createWindow() {
		Window outWindow = new Window("Ripla Demo Application");
	...
	protected IAppConfiguration getAppConfiguration() {
		return new IAppConfiguration() {			
			...
			public IAuthenticator getLoginAuthenticator() {
				if (Boolean.parseBoolean(getPreferences().get(Constants.KEY_LOGIN, Boolean.FALSE.toString()))) {
					return new IAuthenticator() {
						@Override
						public User authenticate(String inName, String inPassword, UserAdmin inUserAdmin) throws LoginException {
							User outUser = inUserAdmin.getUser(Constants.KEY_USER, inName);
							if (outUser == null) {
								throw new LoginException(Activator.getMessages().getFormattedMessage("login.failure.name", inName));
							}
							if (!outUser.hasCredential(Constants.KEY_PW, inPassword)) {
	...
	/* (non-Javadoc)
	 * @see org.ripla.web.RiplaApplication#setUserAdmin(org.osgi.service.useradmin.UserAdmin)
	 */
	@Override
	public void setUserAdmin(UserAdmin inUserAdmin) {
		super.setUserAdmin(inUserAdmin);
		
		User lAdmin = (User) inUserAdmin.createRole(Constants.USER_NAME_ADMIN, Role.USER);
		setNamePassword(lAdmin, Constants.USER_NAME_ADMIN, Constants.USER_PW_ADMIN);
		
		User lUser = (User) inUserAdmin.createRole(Constants.USER_NAME_USER, Role.USER);
		setNamePassword(lUser, Constants.USER_NAME_USER, Constants.USER_PW_USER);
		
		Group lAdministrators = (Group) inUserAdmin.createRole(Constants.ADMIN_GROUP_NAME, Role.GROUP);
	...
	private void setNamePassword(User inUser, String inName, String inPass) {
		if (inUser == null) {
			return;
		}
