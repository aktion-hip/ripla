...
/**
 * UserAdminEventProducer is responsible for sending out UserAdminEvents to all UserAdminListeners.
 * 
 * @author Luthiger
 */
public class UserAdminEventProducer extends ServiceTracker implements EventDispatcher {
	...
	protected ServiceReference<? extends UserAdmin> userAdmin;
	/** List of UserAdminListeners */
	protected CopyOnWriteIdentityMap<ServiceReference<UserAdmin>, ServiceReference<UserAdmin>> listeners;
	/** EventManager for event delivery. */
	protected EventManager eventManager;

	public UserAdminEventProducer(ServiceReference<? extends UserAdmin> inUserAdminRef, BundleContext inContext) {
		super(inContext, USER_ADMIN_LISTENER_CLASS, null);
		
		userAdmin = inUserAdminRef;
		
		ThreadGroup eventGroup = new ThreadGroup("Ripla User Admin"); //$NON-NLS-1$
	...
	public void generateEvent(int inType, Role inRole) {
		if (userAdmin != null) {
			UserAdminEvent lEvent = new UserAdminEvent(userAdmin, inType, inRole);

			/* queue to hold set of listeners */
			ListenerQueue<ServiceReference<UserAdmin>, ServiceReference<UserAdmin>, UserAdminEvent> queue = new ListenerQueue<ServiceReference<UserAdmin>, ServiceReference<UserAdmin>, UserAdminEvent>(eventManager);
	 ...
	 * methods.
	 *
	 * @param inReference Reference to service being added to the <tt>ServiceTracker</tt> object.
	 * @return The service object to be tracked for the
	 * <tt>ServiceReference</tt> object or <tt>null</tt> if the <tt>ServiceReference</tt> object should not
	 * be tracked.
	 */
	public UserAdmin addingService(ServiceReference<UserAdmin> inReference) {
		UserAdmin outService = (UserAdmin) super.addingService(inReference);
	 ...
	 *
	 * @param inReference Reference to service that has been removed.
	 * @param inService The service object for the removed service.
	 */
	public void removedService(ServiceReference inReference, Object inService) {
	 ...
	 *
	 * @param inListener This listener must be cast to the appropriate listener
	 * class for the events created by this source and the appropriate listener method
	 * must then be called.
	 * @param inListenerObject This is the optional object that was passed to
	 * ListenerList.addListener when the listener was added to the ListenerList.
	 * @param inEventAction This value was passed to the EventQueue object via one of its
	 ...
	 * @param inEventObject This object was passed to the EventQueue object via one of its
	 ...
	 * can complete the delivery of the event to the listener.
	 */
	public void dispatchEvent(Object inListener, Object inListenerObject, int inEventAction, Object inEventObject) {
		if (userAdmin == null) {
			return;
		}

		UserAdminListener lUserAdminListener = (UserAdminListener) inListener;
		try {
