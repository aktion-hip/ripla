...
public class ContextMenuItem implements IContextMenuItem {
	 ...
	 * 
	 * @param inControllerClass {@link IPluggable}
	 * @param inMsgKey String the key for the context menu item's label
	 * @param inMenuPermission String
	 * @param inMessages {@link IMessages} the messages to display the language sensitive label
	 */
	public ContextMenuItem(Class<? extends IPluggable> inControllerClass, String inMsgKey, String inMenuPermission, IMessages inMessages) {
	...
	/**
	 * Default implementation of the check of the conditions to display the context menu item.<br />
	 ...
	 */
	@Override
	public boolean checkConditions(User inUser, Authorization inAuthorization,
			ParameterObject inParameters) {
		if (inAuthorization != null && getMenuPermission().length() > 0) {
			if (!inAuthorization.hasRole(getMenuPermission())) {
				return false;
			}
		}
		return true;
